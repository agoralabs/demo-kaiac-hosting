...
export default function SubscriptionPaymentModal({ isOpen, onClose, plan, onSuccess }) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
        {paymentSuccess ? (
          <div className="text-center">
            ...
            <p className="mb-4">Votre paiement de {plan?.totalAmountTTC} {plan?.currency} a été accepté.</p>
          ...
          <>
            <h2 className="text-xl font-bold mb-4">Formule {plan?.name}</h2>
            <div className="mb-4 p-4 bg-gray-50 rounded-md">
                <p className="font-semibold">Montant TTC à régler : {plan?.totalAmountTTC} {plan?.currency}</p>
              </div>
            {/* Ajoutez l'adresse dans l'affichage de confirmation*/}
          <div className="mb-6">
            <h4 className="font-medium mb-2">Adresse de facturation</h4>
            <p className="text-sm">
              {plan.billingAddress.street}<br />
              {plan.billingAddress.postalCode} {plan.billingAddress.city}<br />
