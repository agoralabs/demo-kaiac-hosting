...
export default function MobileMoneyPaymentModal({ isOpen, onClose, plan, onSuccess, userConnected }) {
  ...
  const initializePayment = async () => {
    try {
      ...
      // Créer une transaction via notre API backend
      const response = await api.post('/api/payment/mobile-money/initialize', {
        plan_id: plan.id,
        ...
        customer_phone: plan.billingAddress?.phone || ''
      });
      ...
      // Ouvrir la fenêtre de paiement CinetPay
  ...
  const checkPaymentStatus = async (transId) => {
    try {
      ...
      const checkInterval = setInterval(async () => {
        const response = await api.get(`/api/payment/mobile-money/status/${transId}`);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h3 className="text-xl font-bold mb-4">Paiement Mobile Money</h3>
          ...
          <>
            ...
            <div className="mb-6 text-center">
              {processing ? (
                ...
                <div>
                  <p className="mb-4">Une fenêtre de paiement a été ouverte. Veuillez suivre les instructions pour compléter votre paiement.</p>
                  <button
