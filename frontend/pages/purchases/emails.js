...
export default function Emails() {
    ...
    const fetchData = async () => {
      try {
        const [subsResponse, domainsResponse, emailsResponse] = await Promise.all([
          api.get('/api/user/subscriptions/email'),
  ...
  const handleCreateEmail = async (e) => {
    e.preventDefault();
    try {
      const response = await api.post('/api/domains/create-mailbox', {
        ...formData,
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await api.delete(`/api/domains/delete-mailbox/${emailToDelete.id}`);
